...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    /**
     * Check if provider_id is a provideer
     */
  ...
  async delete(req, res) {
    ...
    if (appointment.user_id !== req.userId) {
      return res
        .status(401)
        .json({ error: "An user can't cancel other user's appointment" });
    }
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res.status(401).json({
        error:
          'User is allowed to cancel appointments with at least 2 hours in advance',
      });
    }
